...
/**
 * Kazda klasa z (pod)pakietu "edu.agh.wsserver" posiadajaca pole public static
 * final String LOG_TAG bedzie dodawana do outputa loggera. Jesli nie chcemy,
...
public class LoggerUpdater extends Thread {
	...
	private static final String SOAP_SERVER_TAG = "GSoapServer";
	private static final String DEF_LOG_LEVEL = "D";
	...
	private String prepareLogcatQuery() {
		...
		List<String> appTags = ServerUtils.getApplicationLogTags(this.mainActivity.getApplicationContext(), "LOG_TAG", "edu.agh.wsserver");
		appTags.add(SOAP_SERVER_TAG);
		for (String appTag : appTags) {
			if (!appTag.equals("")) {
				returnStatement += " " + appTag + ":" + DEF_LOG_LEVEL;
			}
		}
		returnStatement += " *:S";
